...
try:
  ...
  #dllfolder = os.path.join( dllfolder)
  #sys.path.append('apps/python/Spotter/DLLs')
  sys.path.insert(0, dllfolder)
  #sys.path.append('apps/python/Spotter/third_party')
  import ctypes
  #
  if platform.architecture()[0] == "64bit":
  ...
  from third_party import pyfmodex
  #import pyfmodex
...
class Vec3f(tuple):
  ...
  def distance(self,other):
      assert isinstance(other,Vec3f), "Attempted to find distance between non-Vec3 {} with Vec3 {}".format(other,self)
    ...
    def _worker(self):
        while True:
            ...
            while queue_len > 0:
                ...
                if self.sound_mode == pyfmodex.constants.FMOD_3D and self.queue[0]['mode'] == pyfmodex.constants.FMOD_3D:
                    self.channel.position = self.playbackpos
                elif self.sound_mode == pyfmodex.constants.FMOD_2D and self.queue[0]['mode'] == pyfmodex.constants.FMOD_2D:
...
def spotter_stopped(player, drivers, deltaT):
  ...
  if segstarted >=1 and (player.velocity.length() > float(config['Stop']['speed'])): # know if the player's started driving. avoids grid spam.
    for d in drivers:
...
def spotter_slow(player, drivers, deltaT):
  ...
  if segstarted >=1 and (player.velocity.length() > float(config['Stop']['speed'])): # know if the player's started driving.
    for d in drivers:
      if d.velocity.length() < 0.01*float(slowratio)*segdata[d.segment].velocity.length(): # if the driver's going slow
        scan_comp = 1.0-0.75*d.velocity.length()/segdata[d.segment].velocity.length() # and is ahead of the player
